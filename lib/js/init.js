 ...
 * build final html:
 * add jsplumb back
 * look at the actual tool that controls the end product
 * 
 * 
 * 
 * start the thing up
...
function init() {
    _checkLinkable = new storeLinkable(); // are we linking the stage icons or are we just moving them around
    _storeTargets = new storeTargets(); // the list of the end targets for jsplumb
    _storeSources = new storeSource(); // the list of the start targets for jsplumb
    _storeStageIconID = new keepStageIconID();
    _keepStageListID = new keepStageListID();//keep track for removal of the link list items
    _taskListNum = new keepTaskNumber();
...
/*
 * the system checks to see if _projectName is empty
 * if it is it forces the user to give a name
 * I will populate the _projectName when I load a project - before this is fired off
 */
function giveProjectName() {
    if (_projectName == null) {
        var projName = prompt("Please name your project", "");
        if (projName != null) {
...
function buildLinkList(first, second) {
    ...
    if (first != "") {
        $("#linksList").append('<tr id="linkListItemID_' + _keepStageListID.currId + '" class="linkListItems"><td>' + first + ' </td><td>&rarr;</td><td>' + second + '</td></tr>');
        _keepStageListID.currId++;
    }
}
/*
...
function makeDroppableTarget(obj) {
    $(obj).droppable({
        drop: function (event, ui) {
            ...
            _storeSources.source.push(first); // the one being dragged
            _storeTargets.target.push(second);// the one being dropped on
...
function removeLinkListArrayItem(obj) {
    ...
    _storeTargets.target.splice(theNumber, 1);
    _storeSources.source.splice(theNumber, 1);
}
/*
...
function checkForLinking() {
    ...
    if (_checkLinkable.canLink) {
        _checkLinkable.canLink = false;
    } else {
        _checkLinkable.canLink = true;
    }
...
function addTasks() {
    $("#taskListDialog").dblclick(function () {
        var theItems = '<div class="tlItem" id="tli_' + _taskListNum.taskListNum + '"><input type="text" class="taskListStepTitle" placeholder="Enter step"><br><textarea rows="4"  class="taskListStepBody" placeholder="Enter any extra information"></textarea><br></div>';
        $("#taskListItems").append(theItems);
        $("#taskListItems").scrollTop($("#taskListItems").height());
        _taskListNum.taskListNum++;
    });
}
/*
...
function addItems() {
    ...
    $("#iconsDialog").on('click', 'span', function () {
        ...
        $("#stageIconsContainer").append('<span id="si_' + _storeStageIconID.currId + '" class="stageIcons ' + theURL + '"></span>');
        //  $("#container").append('<div class="stageIconCont"><div class="stageIconTxt"></div><img class="stageIcons" src="' + theURL + '"></div>');
        $(".stageIcons").draggable();
        _storeStageIconID.currId++; // increment the _storeStageIconID.currId  by one each time a new icon is added
    });
...
function saving() {
    ...
    $.post('makeDirectories.php', {projectName: _projectName}, function () {
    }).success(function () {

        saveToRawFile('currId', _storeStageIconID.currId, "txt");
        saveToRawFile('objectiveMap', theObjectiveMap, "txt");
        saveToRawFile('stageIcons', theStageIconsContainer, "txt");
        saveToRawFile('targets', _storeTargets.target.join(), "txt");
        saveToRawFile('sources', _storeSources.source.join(), "txt");
        saveToRawFile('tasks', getTasks(), "txt");
        saveToRawFile('taskListCount', _taskListNum.taskListNum, "txt");//so I can keep unique ids


    }).fail(function () {
        console.log('Did not build directory');
    });
}
...
function saveToRawFile(title, data, type) {
    ...
    $.post('saveFile.php', {projectName: _projectName + "_raw", saveFileName: title, projectBody: data, projectFileType: type}, function () {
    }).success(function () {
        console.log('saved');
    }).fail(function () {
...
function placeProjectParts(projName, loading, loadTo, sourceTar) {
    _projectName = projName; // set the global project name

    $.get("projects/" + _projectName + "/" + loading + ".txt", function (data) {
...
function rebuildLinkList() {
    ...
    for (var i = 0; i < _storeTargets.target.length; i++) {
        //get the item by the id number stored in target and sorce
        //make the display item the texdt from the stage item
        var theSourceName = $("#" + _storeSources.source[i]).text();
        var theTargetName = $("#" + _storeTargets.target[i]).text();
...
function buildHTML() {
    ...
    html += '<script>';
    html += '$(".dragged").draggable({ stack: "#container div" });$("#taskListHeader").show().siblings().hide();$("#taskListBody").find("input").eq(1).css("font-size","16px");$(".tlItem:not(:first)").hide();$("#objectiveMapHeader").show().siblings().hide();$("#objectiveMapHeader").dblclick(function(){$("#objectiveMapBody").slideToggle("fast",function(){$("#objectiveMapHeader").toggleClass("dialogLargeOpen", 100, "easeOutSine" );$("#objectiveMapHeader").toggleClass("dialogClosed");});});$("#taskListHeader").dblclick(function(){$("#taskListBody").slideToggle("fast",function(){$("#taskListHeader").toggleClass("dialogClosed");$("#taskListHeader").toggleClass("dialogSmallOpen", 100, "easeOutSine" );});});';
    html += 'var sourceId_arr = ["' + _storeSources.source.join('","') + '"];';
    html += 'var targetId_arr = ["' + _storeTargets.target.join('","') + '"];';
    ...
    html += '\r';
    html += '\r';
    html += 'var objectiveListLinks = 1;function showObjectiveLinks(){$("#obl_"+objectiveListLinks).show();$("#tli_"+objectiveListLinks).show();objectiveListLinks++;}';
    html += '$(".omLinks").html("");';
    html += 'function connectPorts(start,end){showObjectiveLinks();jsPlumb.connect({source:start,target:end,anchors:["Right", "Left" ],endpoint:"Blank",connector: [ "Bezier", { curviness:25 } ],paintStyle:{ lineWidth:1, strokeStyle:"green" }});} ';
    html += '$(".stageIconsConnectorsIndivid").each(function(){var theSRC = $(this).attr("src");$(this).attr("src", theSRC.replace("images/stageIconsConnectors","../lib/images"));});';
    ...
    for (var j = 0; j < _storeSources.source.length; j++) {
        html += ' $("#' + _storeTargets.target[j] + '").droppable({';
        html += 'accept: "#' + _storeSources.source[j] + '",';
        html += 'drop: function (event, ui) {';
        html += 'var draggableId = ui.draggable.attr("id");';
        html += 'var droppableId = $(this).attr("id");';
        // html += 'console.log(draggableId);';
        // html += 'console.log(droppableId);';
    ...
    html += 'var mergeArr = $.merge( sourceId_arr, targetId_arr ),uniqueArr = $.unique(mergeArr);for(var i=0;i<uniqueArr.length;i++){$("#"+uniqueArr[i]).draggable({ opacity: 0.7, helper: "clone" });}';
    html += '});';
    html += '}';
    html += '</script>';
