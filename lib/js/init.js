...
function giveProjectName() {
    if (_startSFB._projectName == null) {
        ...
        if (projName != null) {
            _startSFB._projectName = projName;
        }
    }
}
/*
...
function buildLinkList(first, second) {
    ...
    if (first != "") {
        $("#linksList").append('<tr id="linkListItemID_' + _startSFB._keepStageListID.currId + '" class="linkListItems"><td>' + first + ' </td><td>&rarr;</td><td>' + second + '</td></tr>');
...
function saving() {
    ...
    $.post('makeDirectories.php', {projectName: _startSFB._projectName}, function () {
    }).success(function () {
...
function saveToRawFile(title, data, type) {
    ...
    $.post('saveFile.php', {projectName: _startSFB._projectName + "_raw", saveFileName: title, projectBody: data, projectFileType: type}, function () {
    }).success(function () {
        console.log('saved');
    }).fail(function () {
...
function rebuildLinkList() {
    ...
    for (var i = 0; i < _startSFB._storeTargets.target.length; i++) {
        ...
        var theTargetName = $("#" + _startSFB._storeTargets.target[i]).text();
